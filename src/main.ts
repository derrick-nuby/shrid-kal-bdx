...
async function bootstrap() {
  ...
  const productionServer =
    process.env.PRODUCTION_SERVER ?? 'https://nn.onrender.com';

  const config = new DocumentBuilder()
    .setTitle('Shrid Kal BDX API')
    .setDescription(
      'This API provides endpoints for managing tasks and other functionalities within the Shrid Kal BDX application.',
    )
    .setVersion('1.0')
